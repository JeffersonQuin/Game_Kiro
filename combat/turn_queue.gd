...
func play_turn() -> void:
	_turn_loop_active = true
	while _turn_loop_active and active_combatant:
		await active_combatant.turn_finished
		if not _turn_loop_active or not is_instance_valid(active_combatant):
			break
		get_next_in_queue()


func stop() -> void:
	_turn_loop_active = false
